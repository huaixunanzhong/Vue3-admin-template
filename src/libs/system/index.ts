 ...
 * */
function getHeaderName (currentPath:any, menuList:any) {
    const allMenus:any[] = [];
...
function transferMenu (menu:any, headerName:any) {
 ...
 * */
function getMenuSider (menuList:any, headerName = '') {
    if (headerName) {
 ...
 * @param {Array} menuList 所有路径
 * */
function getSiderSubmenu (currentPath:any, menuList:any) {
    const allMenus:any[] = [];
    menuList.forEach((menu:any) => {
...
function transferSubMenu (menu:any, openNames:any) {
    if (menu.children && menu.children.length) {
 ...
 * */
function includeArray (list1:any, list2:any) {
    let status = false;
