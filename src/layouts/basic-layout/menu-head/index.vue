        ...
        <Menu mode="horizontal" :active-name="headerName" v-if="!isMobile && !isMenuLimit" ref="menu">
            <template v-for="(item, index) in filterHeader" :key="item.path">
                <Submenu v-if="item.children && item.children.length" :name="item.name" :key="index">
                    ...
                    <MenuItem v-for="subitem in item.children" :to="subitem.path" :replace="subitem.replace" :target="subitem.target" :name="item.name" :key="subitem.path" @click="handleClick(subitem.path, 'header')">
                    ...
                    <DropdownMenu>
                        <template v-for="(item, index) in filterHeader">
                            <template v-if="item.children && item.children.length">
                                ...
                                <i-link v-for="subitem in item.children" :to="subitem.path" :replace="subitem.replace" :target="subitem.target" :key="subitem.path" @click="handleClick(subitem.path, 'header')">
                                    <DropdownItem>
                                        <i-menu-head-title :item="subitem" badge />
                                    </DropdownItem>
                                </i-link>
    ...
    export default defineComponent({
        ...
        methods: {
            handleGetMenuHeight () {
                ...
                if ($menu) {
                    const self:any = this
                    const menuHeight = parseInt(getStyle(self.$refs.menu['$el'], 'height'));
