    ...
    actions: {
        ...
        openedLoad ({ state, commit, dispatch, rootState }:any, { loadOpenedTabs = true }) {
            return new Promise(async resolve => {
                ...
                const valid: any[] = [];
                // 处理数据
                state.opened = value.map((opened:any) => {
                    // 忽略首页
         ...
         */
        openedUpdate ({ state, dispatch }:any, { index, params, query, fullPath, meta, keepMeta = false }:any) {
            return new Promise(async resolve => {
                // 更新页面列表某一项
         ...
         */
        currentUpdate ({ state, dispatch }:any, { params, query, fullPath, meta, keepMeta = false }:any) {
            return new Promise(async resolve => {
                setTimeout(async () => {
                    // 更新当前项
         ...
         */
        add ({ state, commit, dispatch }:any, { tag, params, query, fullPath }:any) {
            return new Promise(async resolve => {
         ...
         */
        open ({ state, commit, dispatch }:any, { name, params, query, fullPath }:any) {
            return new Promise(async resolve => {
                // 已经打开的页面
         ...
         */
        close ({ state, commit, dispatch }:any, { tagName }:any) {
            return new Promise(async resolve => {
                // 下个新的页面
         ...
         */
        closeLeft ({ state, commit, dispatch }:any, { pageSelect }: any = {}) {
            return new Promise(async resolve => {
                ...
                if (currentIndex > 0) {
                    // 删除打开的页面 并在缓存设置中删除
                    state.opened.splice(1, currentIndex - 1).forEach(({ name }:any) => commit('keepAliveRemove', name));
                }
         ...
         * @param {Object} param { pageSelect: 当前选中的tagName }
         */
        closeRight ({ state, commit, dispatch }:any, { pageSelect }: any = {}) {
            return new Promise(async resolve => {
                const pageAim = pageSelect || state.current;
                let currentIndex = 0;
                state.opened.forEach((page:any, index:any) => {
                ...
                // 删除打开的页面 并在缓存设置中删除
                state.opened.splice(currentIndex + 1).forEach(({ name }:any) => commit('keepAliveRemove', name));
                // 设置当前的页面
                state.current = pageAim;
                if (router.currentRoute.value.fullPath !== pageAim) {
                    router.push(pageAim);
         ...
         * @param {Object} param { pageSelect: 当前选中的tagName }
         */
        closeOther ({ state, commit, dispatch }:any, { pageSelect }: any = {}) {
            return new Promise(async resolve => {
                const pageAim = pageSelect || state.current;
                let currentIndex = 0;
                state.opened.forEach((page:any, index:any) => {
